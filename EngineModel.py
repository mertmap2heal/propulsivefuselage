...
class NacelleParameters:
    ...
    def engine_parameters(self, rho, p):
...
class DragbyBLIEngine:
    ...
    def calculate_zero_lift_drag(self):
        ...
        Re = min(Re, Re0)  # Apply cutoff
        
        # 4.7 Calculate skin friction coefficient
        # Calculate Mach number
        gamma = 1.4
        R_air = 287  # Specific gas constant for air (J/kg·K)
        speed_of_sound = math.sqrt(gamma * R_air * self.T)
        M_infty = self.v_freestream / speed_of_sound
        
        # Simple if-else regime selection
        if Re < 2e5:  # Laminar region
            cf = 1.328 / math.sqrt(Re)
        else:  # Turbulent region
            cf = 0.455 / (math.log10(Re))**2.58 * (1 + 0.144 * M_infty**2)**(-0.65)
        
        # 4.8 Calculate form factor adjustment
        f = self.nac_length / (2 * self.inlet_radius)  # Fineness ratio
        Fnac = 1 + 0.35 / f  # Form factor
...
class Flow_around_fuselage:
    def __init__(self, flight_conditions, nacelle_params, FL, v_freestream, Mach, rho, mu, delta_p, A_inlet, p,
        ...
        nacelle_params = self.nacelle.engine_parameters(rho=self.rho, p=self.p)
...
class BoundaryLayerIngestion:
    ...
    def _process_nacelle_geometry(self, rho, p, v_inlet):
        ...
        results_nacelle = nacelle.engine_parameters(rho, p)
    ...
    def _process_engine_mass_estimation(self, v_inlet, rho, p, nacelle):
        ...
        nacelle.engine_parameters(rho, p)  # Ensure parameters are updated
